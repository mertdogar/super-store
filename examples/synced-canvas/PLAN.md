# Plan: `examples/synced-canvas` — a super-store collaboration showcase
...
The original syncs by relaying **Yjs update bytes** over super-line. super-store _is_ Yjs
...
`local` means **"not injected via `applyUpdate`"** — so user edits (STORE_ORIGIN) _and_
...
- Location `examples/synced-canvas` (the `examples/*` workspace glob).
