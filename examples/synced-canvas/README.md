...
- **Per-field CRDT merge.** Each shape is its **own nested `StoreValue`**, so one user dragging a shape while another recolors the _same_ shape merges per field instead of clobbering.
...
- **Undo/redo** (super-store, opt-in): only _your_ edits revert — remote merges are never undone — and an undo propagates to peers like any edit.
