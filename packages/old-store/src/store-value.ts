...
function getTypeName<T>(value: T): (typeof supportedTypes)[number] {
  ...
  if (typeof value === "object") {
    ...
    if (specialTypes.includes(name as never))
      return name as (typeof specialTypes)[number]
...
export class StoreValue<T> {
  ...
  private _buildSnapshot(): InferStoreValueSnapshot<T> {
    if (this._childUnsubs.length === 0)
      return this._value as InferStoreValueSnapshot<T>
